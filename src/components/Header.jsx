    ...
    <header className="flex items-center justify-between px-4 sm:px-5 py-3 bg-slate-800/80 dark:bg-slate-900/90 border-b border-slate-700 z-40">
        ...
        <Link to="/" className="flex items-center gap-3 no-underline">
          ...
          <div className="leading-tight hidden sm:block">
            <div className="text-lg font-semibold text-slate-100">
              EventMaster <span className="text-sky-400 text-lg ml-1 font-normal">Playground</span>
            </div>
          </div>
        </Link>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          onClick={() => {
            // prefer parent handler if provided, fallback to theme-flash
            if (onReset) {
              onReset();
              return;
            }
        ...
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className="opacity-90">
            <path d="M21 12a9 9 0 1 1-3-6.7" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
            <path d="M21 3v6h-6" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
          Reset
        </button>

        <button aria-label="Toggle color theme" onClick={toggleTheme} className="p-2 rounded-md hover:bg-slate-700/30">
          {theme === "dark" ? (
            ...
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
              <path d="M21 12.8A9 9 0 1 1 11.2 3 7 7 0 0 0 21 12.8z" stroke="#93C5FD" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          )}
        </button>
      </div>
